# Security policy — fastx_dashboards
...
- Do not commit secrets, API keys, or tokens; use environment variables or a secrets manager ([`fastx_secrets`](../fastx_secrets/README.md) integrates Vault / cloud backends in FastMVC apps).
- Review `pyproject.toml` and lock files in production deployments.

In the FastMVC stack, the sibling package **fastx_secrets** provides optional Vault / cloud secret backends for applications (see the monorepo [README](../README.md)).
