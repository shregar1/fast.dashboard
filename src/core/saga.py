...
class SagaStep:
    ...
    compensation: Optional[Callable[[SagaContext, Any], Coroutine[Any, Any, None]]] = None
    retry_count: int = 0
...
class Saga:
    ...
    def on_status_change(
        self,
        callback: Callable[[SagaExecution], None]
    ) -> Saga:
        """Set a callback for status changes."""
        self._on_status_change = callback
        return self
    
    async def execute(self, initial_data: Optional[Dict[str, Any]] = None) -> SagaExecution:
        """
        Execute the saga.
            ...
            for step in sequential_steps:
                ...
                if step.condition and not step.condition(context):
                    execution.steps.append({
                        "name": step.name,
                        "status": SagaStepStatus.SKIPPED.value
                    })
                    continue
            ...
            try:
                ...
                if result.success:
                    ...
                    if result.compensation_data:
                        context.set(f"_compensation_{step.name}", result.compensation_data)
    ...
    async def _compensate_steps(
        ...
        if self.parallel_compensation:
            ...
            await asyncio.gather(*[
                self._compensate_step(step, context)
