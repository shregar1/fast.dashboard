...
class RecordingStore:
    ...
    def complete(self, recording_id: str, response_data: Any, error: Optional[str] = None):
        """Mark a recording as complete."""
        recording = self._active_recordings.get(recording_id)
        if recording:
            recording.status = RecordingStatus.FAILED if error else RecordingStatus.COMPLETED
    ...
    def record(
        ...
        if inspect.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                recording.request_data = {
                    "args": repr(args),
                    "kwargs": repr(kwargs)
                }
                
                # Create tracer
                tracer = _ExecutionTracer(recording, capture_db, capture_cache, capture_external)
    ...
    def _capture_snapshot(self, frame):
        ...
        try:
            ...
            snapshot = Snapshot(
                ...
                global_vars={k: v for k, v in frame.f_globals.items() if not k.startswith('_')},
                call_stack=stack,
...
class TimeTravelCLI:
    ...
    def replay_recording(
        recording_id: str,
        breakpoint_line: Optional[int] = None
    ):
        """Replay a recording."""
        debugger = TimeTravelDebugger()
